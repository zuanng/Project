...
class Command(BaseCommand):
    help = "Tạo dữ liệu đơn hàng lịch sử trong 2 tháng qua"

    def handle(self, *args, **options):
        # Lấy 5 user đã có (hoặc tạo mới nếu chưa có)
        ...
        # Xóa các đơn hàng được tạo bởi lệnh này trước đó (để tránh trùng lặp)
        Order.objects.filter(order_number__startswith='HIST').delete()
        self.stdout.write("Đã xóa dữ liệu đơn hàng lịch sử cũ (nếu có)")

        # Tạo đơn hàng phân bố trong 2 tháng qua (không bao gồm ngày hôm nay)
        current_time = timezone.now()
        orders_created = 0
        total_revenue = Decimal('0')
        
        # Tạo 120 đơn hàng phân bố đều trong 60 ngày (2 tháng), trừ ngày hôm nay
        for _ in range(120):
            # Chọn ngẫu nhiên một user
            user = random.choice(users)
            
            # Chọn ngẫu nhiên một ngày trong khoảng 2-60 ngày trước (để tránh ngày hôm nay)
            random_days_back = random.randint(2, 60)  # 2 to 60 days back (not including today)
            base_date = current_time.date() - timedelta(days=random_days_back)
            ...
            order_time = timezone.make_aware(naive_datetime)
            
            # Tạo đơn hàng với số đơn hàng duy nhất
            import time
            while True:
                order_number = f"HIST{int(time.time())}{random.randint(10000, 99999)}"
                if not Order.objects.filter(order_number=order_number).exists():
                    break
            
            with transaction.atomic():
                # Create order without forcing the timestamps initially
                order = Order(
                ...
                # Bypass auto timestamps to allow historical dates
                order.save()
                
                # Manually update the timestamps with historical dates
                Order.objects.filter(pk=order.pk).update(
                    created_at=order_time,
                    updated_at=order_time
                )
                
                # Refresh the instance to get the updated timestamps
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"\nTạo thành công {orders_created} đơn hàng lịch sử đã hoàn thành và đã thanh toán "
                f"cho 5 user khác nhau trong 2 tháng qua (không bao gồm hôm nay)."
            )
        )
        
        self.stdout.write(
            self.style.SUCCESS(
                f"Tổng doanh thu từ các đơn hàng lịch sử: {total_revenue:,}đ"
            )
        )